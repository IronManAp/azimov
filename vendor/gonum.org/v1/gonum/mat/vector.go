...
func (v *VecDense) SumLog() float64 {
	sum_log := 0.0
	prod := 1.0

	for i := 0; i < v.RawVector().N; i++ {
		prod *= v.RawVector().Data[i]

		if prod < 1.0e-10 || prod > 1.0e+10 {
			sum_log += math.Log(prod)
			prod = 1.0
		}
	}
...
func (v *VecDense) ApplyFloor(var_floor float64) {
	v.mat.Floor(&v.mat, var_floor)
}

func (v *VecDense) InvertElement() {
	for i := 0; i < v.RawVector().N; i++ {
		v.RawVector().Data[i] = 1.0 / v.RawVector().Data[i]
	}
}

func (v *VecDense) Min() float64 {
	ans := math.Inf(1)
	var i int

	for i = 0; i+4 <= v.RawVector().N; i += 4 {
		a1 := v.RawVector().Data[i]
		a2 := v.RawVector().Data[i+1]
		a3 := v.RawVector().Data[i+2]
		a4 := v.RawVector().Data[i+3]
	...
	for ; i < v.RawVector().N; i++ {
		if v.RawVector().Data[i] < ans {
			ans = v.RawVector().Data[i]
		}
	}
